...
echo $blade->run("room-details", ["room" => $result->fetch_array(), "related" => $result2->fetch_all(), "getAvailable" => getAvailability($conn, $id)]);
...
function getAvailability($conn, $id)
{
    if ($_POST["checkin"] && $_POST["checkout"]) {
        ...
        if ($result->fetch_array()[0] == null || $result->fetch_array()[0] == 0) {
            echo "Available";
        }else{
            echo "Booked";
        }
